...
class JacoServeEnv(JacoEnv):
    def __init__(self, with_rot=0, reward_type='dense',distance_threshold=0.05):
    ...
    def _get_obs(self):
        ...
        # ob = np.concatenate([qpos, np.clip(qvel, -30, 30), qacc, hand_pos]).ravel()
        ob = np.concatenate([grip_pos, object_pos.ravel(), object_rel_pos.ravel(), gripper_state, object_rot.ravel(),
            object_velp.ravel(), object_velr.ravel(), np.clip(qvel, -30, 30), qacc])
        return {
            'observation': ob.copy(),
